...
void hashT::printInfo() const {
    int count = 1;
    for (int i = 0; i < HTSize; i++)
        if (indexStatusList[i] != 0 && indexStatusList[i] != -1)
            std::cout << hTable[i].word << " " << hTable[i].count << " " << i << std::endl;
}

int hashT::hashFunction(const std::string& key) {
    size_t sum = 0;
